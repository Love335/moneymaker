...
class Engine:
    ...
    def _on_shutdown_requested(self, event: Event) -> None:
        logger.info("Shutdown requested: %s", event.payload.get("reason"))
    ...
    def _on_yes_for_recovery(self, event: Event) -> None:
        """YES button confirms recovery from unrecoverable crash."""
        snap = self._state.snapshot()
        if snap.system_status == SystemStatus.AWAITING_RECOVERY:
            logger.info("Recovery confirmed by user")
    ...
    def _perform_shutdown(self) -> None:
        ...
        self._running = False
        self._state.set_status(SystemStatus.SHUTTING_DOWN)
        self._display.show_message("SHUTTING DOWN")
        self._led.set_state(LEDState.WORKING)
        ...
        for subsystem_name, subsystem in [
            ...
            if subsystem:
                try:
                    ...
                    logger.exception("Error stopping %s", subsystem_name)
        ...
        # Trigger OS shutdown via PowerManager
        if self._power:
            self._power.initiate_shutdown()
